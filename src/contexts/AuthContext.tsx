...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const setupLoadingTimeout = () => {
      ...
      loadingTimeoutRef.current = setTimeout(() => {
        if (mounted && isLoading) {
          console.warn('Authentication process stuck in loading state - resetting to unauthenticated state');
          
          // Reset state directly without calling logout to avoid potential issues with invalid sessions
          setIsLoading(false);
          setUser(null);
          setProfile(null);
          setSession(null);
  ...
  const login = async (email: string, password: string) => {
    ...
    // Note: Removed the setTimeout that was setting isLoading to false
    // Let the onAuthStateChange listener handle the loading state
  };
